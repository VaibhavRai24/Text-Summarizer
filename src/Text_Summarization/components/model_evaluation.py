...
from Text_Summarization.entity.DataIngestionConfig import ModelEvaluatorConfig
...
class ModelEvaluation:
    ...
    def calculate_metric_on_test_df(self, dataset, metric, model, tokenizer,
                                    batch_size = 16, device = "cuda" if torch.cuda.is_available else "cpu",
                                    column_text = "article", column_summary = "highlights"):
        """calculate the metric on the test dataset"""
        articles_batches = list(self.generate_batch_size_chunks(dataset[column_text], batch_size))
        target_batches = list(self.generate_batch_size_chunks(dataset[column_summary], batch_size))\
        
        for article_batch , target_batch in tqdm(
            zip(articles_batches, target_batches), total=len(articles_batches)):
    ...
    def evaluate(self):
        ...
        score = self.calculate_metric_on_test_df(
